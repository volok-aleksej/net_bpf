...
static __always_inline int process_ipv4_redirect(struct ebpf_context *ctx, struct __sk_buff *skb,
                                            __u32 id, struct Config* icfg, __u32 is_fragment)
{
...
static __always_inline int process_ipv6_redirect(struct ebpf_context *ctx, struct __sk_buff *skb,
                                            __u32 id, struct Config* icfg, __u32 is_fragment)
{
    if(!is_fragment && check_need_fragmentation(skb, icfg)) {
        is_fragment = 1;
...
static __always_inline int process_tcpudp_packet(struct ebpf_context *ctx, struct __sk_buff *skb, __u16 off, __u8 protocol, struct Config** cfg_)
{
    ...
    if(protocol == IPPROTO_UDP) {
        ...
        ctx->payload_size = htons(udph.len) - sizeof(struct udphdr);
...
static __always_inline int process_ipv4_packet(struct ebpf_context *ctx, struct __sk_buff *skb, __u16 off)
{
    ...
    if(ret == TC_ACT_REDIRECT) {
        return process_ipv4_redirect(ctx, skb, ctx->int_id, cfg_, ctx->is_fragment);
    }
    return ret;
}
...
static __always_inline int process_ipv6_packet(struct ebpf_context *ctx, struct __sk_buff *skb, __u16 off)
{
    ...
    for(int i= 0; i < MAX_IP6_EXT_HDR && off < data_end - data; i++) {
        if(next_proto == IPPROTO_FRAGMENT) {
            ...
            if(ret == TC_ACT_REDIRECT)
                return process_ipv6_redirect(ctx, skb, ctx->int_id, cfg_, ctx->is_fragment);
            return ret;    
        } else {
            return TC_ACT_OK;
        }
...
{
    int i = 0;
    struct ebpf_context *ctx = bpf_map_lookup_elem(&BPF_CONTEXT_MAP, &i);
    ...
    if (eth_proto == ETH_P_IP) {
        //clear context
        bpf_map_delete_elem(&BPF_CONTEXT_MAP, &i);
...
int BPF_IPv4_RDR_FUNCTION(struct __sk_buff *skb)
{
    ...
    struct Config* cfg_ = bpf_map_lookup_elem(&BPF_CFG_MAP, &ctx->int_id);
    ...
    return redirect_ip4_fragment(ctx, skb, ctx->cur_info.if_index, ctx->cur_info.node_id);
}
...
int BPF_IPv6_RDR_FUNCTION(struct __sk_buff *skb)
{
    ...
    return redirect_ip6_fragment(ctx, skb, ctx->cur_info.if_index, ctx->cur_info.node_id);
}
